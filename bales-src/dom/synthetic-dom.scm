...
(define (youtube . args) ; (youtube seml-spec width height url) returns an iframe string to embed the specified video. The seml spec is optional.
  (let ((nargs (length args)))
    (cond
      ((eqv? nargs 4)
            (iframe (<< (car args) ".youtube!frameborder=0!allowfullscreen=true!width=" (cadr args) "!height=" (caddr args) "!src='" (cadddr args) "'") "YouTube video"))
      ((eqv? nargs 3)
            (iframe (<< ".youtube!frameborder=0!allowfullscreen=true!width=" (cadr args) "!height=" (car args) "!src='" (caddr args) "'") "YouTube video"))
    (else "YouTube format error"))))
